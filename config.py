...
class Config:
    ...
    def set_up(cls):
        ...
        parser = argparse.ArgumentParser(description='szczepienia.pw end-to-end tests')
        ...
        parser.add_argument('--headless', help='display browser window', action=argparse.BooleanOptionalAction)
        ...
        Mailbox.debug(f"args were: {cls.args}")
        Mailbox.info(f"Will test backend at: {cls.backUrl} and frontend at {cls.frontUrl}, test will be run {'non-' if not cls.headless else ''}headless")
